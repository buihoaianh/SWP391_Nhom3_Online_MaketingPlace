    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (service.equals("listMembers")) {
                ...
                int totalMembers = dao.getTotalMembers();
                int totalPages = (int) Math.ceil((double) totalMembers / pageSize);
                List<Member> list = dao.getMembersByPage(page, pageSize);
            ...
            if (service.equals("insertForm")) {
                ...
                Member newMember = new Member(0, name, minAmount, maxAmount, desc, imageURL);
                boolean success = dao.addMember(newMember);

                response.sendRedirect("MemberURL?service=listMembers");
            } else if (service.equals("updateForm")) {
                ...
                Member updated = new Member(id, name, minAmount, maxAmount, desc, imageURL);
                dao.updateMember(updated);
                response.sendRedirect("MemberURL?service=listMembers");
            } else if (service.equals("deleteMember")) {
                int id = Integer.parseInt(request.getParameter("id"));
                dao.deleteMember(id);
                response.sendRedirect("MemberURL?service=listMembers");
            } else if (service.equals("searchMember")) {
