...
public class sendEmail {//send to email
    ...
    public boolean sendNewPassword(String to, String newPassword, String name) {
        Properties props = new Properties();
        props.put("mail.smtp.host", "smtp.gmail.com");
        props.put("mail.smtp.port", "587");
        props.put("mail.smtp.auth", "true");
        ...
        try {
            ...
            msg.setFrom(new InternetAddress(from));
            msg.setRecipients(Message.RecipientType.TO, InternetAddress.parse(to, false));
            msg.setSubject("Cấp lại mật khẩu mới", "UTF-8");

            String content = "<h3>Chào " + name + "</h3>"
                    + "<p>Mật khẩu mới của bạn là: <strong>" + newPassword + "</strong></p>"
                    + "<p>Hãy đăng nhập và đổi lại mật khẩu để bảo mật tài khoản.</p>";

            msg.setContent(content, "text/html; charset=UTF-8");

            Transport.send(msg);
            System.err.println("Send new password successfully!");
            return true;
        } catch (Exception e) {
            System.err.println("Send new password error");
            e.printStackTrace();
            return false;
        }
    }
