...
public class SellerRequestDAO {
    ...
    public void updateRequestStatus(int requestID, String status, int reviewerID, String reason) throws SQLException {
        String sql = "UPDATE SellerRequests SET Status = ?, ReviewedBy = ?, ReviewDate = GETDATE(), RejectReason = ? WHERE RequestID = ?";
        PreparedStatement ps = ConnectDB.getConnection().prepareStatement(sql);
        ps.setString(1, status);
        ps.setInt(2, reviewerID);
    ...
    public void updateAccountStatusToActive(int requestID) throws SQLException {
        String sql = "UPDATE Account SET Status = 1 WHERE AccountID = (SELECT AccountID FROM SellerRequests WHERE RequestID = ?)";
        PreparedStatement ps = ConnectDB.getConnection().prepareStatement(sql);
        ps.setInt(1, requestID);
        ps.executeUpdate();
    }
    
    //Chỉ chặn nếu đã có request với trạng thái Approve, còn Pending và Reject vẫn được gửi tiếp.
    public boolean hasApprovedRequest(int accountID) throws SQLException {
    String sql = "SELECT COUNT(*) FROM SellerRequests WHERE AccountID = ? AND Status = 'Approve'";
    PreparedStatement ps = ConnectDB.getConnection().prepareStatement(sql);
    ps.setInt(1, accountID);
    ResultSet rs = ps.executeQuery();
    if (rs.next()) {
